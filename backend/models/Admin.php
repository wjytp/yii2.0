...
//        var_dump($model->save(),$model->getErrors());die;
        if($model->save()){
            if(!$this->setAdminRole($id,$this->role)){
                return false;
            }
        }else{
            return false;
    ...
    public function setAdminRole($id,$roleName){
        $authManager = Yii::$app->authManager;
        //删除用户所在的用户组
        $authManager->revokeAll($id);
        //添加用户组
        return $authManager->assign($authManager->getRole($roleName), $id);
    }
}
